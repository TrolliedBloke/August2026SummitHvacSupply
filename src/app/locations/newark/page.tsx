...
function Fact({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) { return <article><div className="flex items-center gap-2 text-ink-1">{icon}<h2 className="font-medium">{title}</h2></div><div className="mt-3 text-sm leading-6 text-ink-2">{children}</div></article>; }
