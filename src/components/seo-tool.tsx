...
function SizingEstimator() {
  ...
  return <div className="rounded-(--r-md) border border-line bg-surface-1 p-5 sm:p-7"><div className="grid gap-5 sm:grid-cols-3"><Field label="Conditioned area"><input className="field" type="number" min="100" max="5000" step="50" value={area} onChange={(event) => setArea(event.target.value)} /></Field><Field label="Rooms or zones"><input className="field" type="number" min="1" max="8" value={rooms} onChange={(event) => setRooms(event.target.value)} /></Field><Field label="Existing ducts"><select className="field" value={ducts} onChange={(event) => setDucts(event.target.value)}><option value="yes">Yes</option><option value="no">No</option><option value="unknown">Not sure</option></select></Field></div><div aria-live="polite" className="mt-6 rounded-(--r-sm) border border-line bg-page p-5"><p className="text-sm text-ink-3">Conversation starting point</p><p className="part-number mt-1 text-3xl font-medium text-ink-1">{Number.isFinite(estimate) ? estimate.toLocaleString() : "-"} BTU</p><p className="mt-2 text-sm leading-6 text-ink-2">For {rooms} room{rooms === "1" ? "" : "s"}, this suggests reviewing {ducts === "no" ? "ductless" : ducts === "yes" ? "ducted or ductless" : "ducted and ductless"} options near this capacity. Solar gain, insulation, ceiling height, leakage, climate, and occupancy can change the result substantially.</p><Link href={`/products?btu=${estimate <= 12000 ? "small" : estimate >= 36000 ? "large" : "mid"}`} className="mt-4 inline-flex text-sm text-ink-1 underline underline-offset-4">Review nearby equipment</Link></div></div>;
}
