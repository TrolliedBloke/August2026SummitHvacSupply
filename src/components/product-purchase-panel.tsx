      ...
      <div className="flex items-end gap-3">
        <label className="block w-28 text-sm text-ink-2">
          ...
          <span className="mt-1 flex h-12 items-center rounded-(--r-sm) border border-line bg-surface-1">
            ...
            <input aria-label="Quantity" inputMode="numeric" value={quantity} onChange={(event) => setQuantity(Math.max(1, Number(event.target.value) || 1))} className="min-w-0 flex-1 bg-transparent text-center font-mono text-sm outline-none" />
            <button type="button" aria-label="Increase quantity" className="grid h-full w-9 place-items-center" onClick={() => setQuantity((value) => value + 1)}><Plus size={15} /></button>
          </span>
        </label>
