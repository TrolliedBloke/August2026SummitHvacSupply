  ...
  return (
    <div>
      <div className="relative aspect-[1.5/1] overflow-hidden rounded-(--r-md) border border-line bg-surface-2 sm:aspect-[1.22/1]">
        ...
        {current.kind === "photo" && (
          <button type="button" onClick={() => setZoomed(true)} aria-label="Open large product image" className="absolute bottom-3 right-3 grid size-10 place-items-center rounded-(--r-sm) border border-line bg-surface-1 text-ink-1">
            <Maximize2 size={17} />
          </button>
        )}
      </div>
      ...
      {zoomed && current.kind === "photo" && (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-[var(--ink-panel)]/90 p-5" role="dialog" aria-modal="true" aria-label="Large product image" onClick={() => setZoomed(false)}>
          <button type="button" aria-label="Close large image" className="absolute right-5 top-5 grid size-11 place-items-center rounded-(--r-sm) bg-surface-1 text-ink-1" onClick={() => setZoomed(false)}><X size={20} /></button>
...
function GalleryButton({ label, className, onClick, children }: { label: string; className: string; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" aria-label={label} onClick={onClick} className={`absolute top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full border border-line bg-surface-1 text-ink-1 ${className}`}>{children}</button>;
}
